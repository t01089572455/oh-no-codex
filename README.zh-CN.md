...
可弃用项目副本证据）。npm 包 `oh-no-codex@0.1.1` 已公开；这**不等于**敌对 Agent 安全产品、
也不等于普适速度保证。
...
| npm 包 `oh-no-codex` | 已发布 `0.1.1`（[registry.npmjs.org](https://www.npmjs.com/package/oh-no-codex)） | `npm install -g oh-no-codex` |
