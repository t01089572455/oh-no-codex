...
需要 Node.js **22.20 或更高版本**。当前版本：**`0.1.11`**。
...
| 公开事实 | 当前证据 |
| --- | --- |
| npm 包 | [`oh-no-codex@0.1.11`](https://www.npmjs.com/package/oh-no-codex) 已发布 |
| 核心闭环 | `ANTI_DRIFT_CORE_WORKS`，有本机公开黑盒覆盖 |
| 计划后自动执行 | 已纳入 `0.1.11`：plan 接受后 Stop 以 `OHNO_AUTO_CONTINUE` 续跑（协作式，非 daemon） |
